 ...
 * Note:
 * 
 * All key/value strings are lowercase. All key/value strings have length in the
 * range [1, 100] The timestamps for all TimeMap.set operations are strictly
...
public class TimeMap {
	...
	Map<String, Map<Integer, String>> bigMap;
	Map<Integer, String> smallMap;
	...
	public void set(String key, String value, int timestamp) {
		if (bigMap.containsKey(key)) {
			smallMap = bigMap.get(key);
			smallMap.put(timestamp, value);
		} else {
			smallMap = new HashMap<>();
			smallMap.put(timestamp, value);
			bigMap.put(key, smallMap);
		}
	}
