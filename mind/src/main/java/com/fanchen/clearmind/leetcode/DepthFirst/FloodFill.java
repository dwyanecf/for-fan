...
public class FloodFill {
	public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
		...
		helper(image, sr, sc, m, n, originColor, newColor);
		return image;

	}

	private void helper(int[][] image, int sr, int sc, int m, int n, int originColor, int newColor) {
		if (sr < 0 || sr >= m || sc < 0 || sc >= n || image[sr][sc] != originColor) {
			return;
		}
		image[sr][sc] = newColor;
		helper(image, sr + 1, sc, m, n, originColor, newColor);
		helper(image, sr - 1, sc, m, n, originColor, newColor);
		helper(image, sr, sc - 1, m, n, originColor, newColor);
		helper(image, sr, sc + 1, m, n, originColor, newColor);
	}
}
