...
public class SubdomainVisitCount {
	public List<String> subdomainVisits(String[] cpdomains) {
		...
		for (String domain : cpdomains) {
			String[] timeAndDomain = domain.split(" ");
			int time = Integer.valueOf(timeAndDomain[0]);
			String domainString = timeAndDomain[1];
			String[] subDomain = getSubDomain(domainString);
			for (String key : subDomain) {
	...
	private String[] getSubDomain(String domainString) {
		List<String> subDomain = new ArrayList<>();
		subDomain.add(domainString);
		for (int i = domainString.length() - 1; i >= 0; i--) {
			if (domainString.charAt(i) == '.') {
				subDomain.add(domainString.substring(i + 1, domainString.length()));
			}
		}
		return subDomain.toArray(new String[subDomain.size()]);
	}
