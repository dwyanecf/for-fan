...
public class LC630CourseSchedule3 {
	
	/**
	 * There are n different online courses numbered from 1 to n. 
	 * Each course has some duration(course length) tand closed on dth day. 
	...
	public int scheduleCourse(int[][] courses) {
		...
		// after sorted: [[100, 200], [1000, 1250], [200, 1300], [2000, 3200]]
		...
		for (int course[] : courses) {
			...
			endingMax += d;
			while (endingMax > e) {
				endingMax -= pq.poll(); // kick off the big during class
			}
		}
		return pq.size();
	}
