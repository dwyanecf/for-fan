 ...
 * 
 * 
 * Example 1: Input: ["root/a 1.txt(abcd) 2.txt(efgh)", "root/c 3.txt(abcd)",
 * "root/c/d 4.txt(efgh)", "root 4.txt(efgh)"] Output:
 * [["root/a/2.txt","root/c/d/4.txt","root/4.txt"],["root/a/1.txt","root/c/3.txt"]]
 * 
 * Note: No order is required for the final output. You may assume the directory
 * name, file name and file content only has letters and digits, and the length
 * of file content is in the range of [1,50]. The number of files given is in
 * the range of [1,20000]. You may assume no files or directories share the same
 * name in the same directory. You may assume each given directory info
 * represents a unique directory. Directory path and file info are separated by
...
public class FindDuplicateFileInSystem {
	...
	public List<List<String>> findDuplicate(String[] paths) {
		...
		for (String each : paths) {
			String[] toks = each.split(" ");

			for (int i = 1; i < toks.length; i++) {
				String file = toks[i].substring(0, toks[i].indexOf('('));
				String content = toks[i].substring(toks[i].indexOf('(') + 1, toks[i].indexOf(')'));
				map.putIfAbsent(content, new ArrayList<String>());
				map.get(content).add(toks[0] + '/' + file);
			}
		}
