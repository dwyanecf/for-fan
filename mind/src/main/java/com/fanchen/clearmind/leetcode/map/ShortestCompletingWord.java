 ...
 * word. Example 2: Input: licensePlate = "1s3 456", words = ["looks", "pest",
 * "stew", "show"] Output: "pest" Explanation: There are 3 smallest length words
 * that contains the letters "s". We return the one that occurred first. Note:
 * licensePlate will be a string with length in range [1, 7]. licensePlate will
 * contain digits, spaces, or letters (uppercase or lowercase). words will have
 * a length in the range [10, 1000]. Every words[i] will consist of lowercase
 * letters, and have length in range [1, 15].
 * 
 * @author fan
 *
 */
public class ShortestCompletingWord {
	public String shortestCompletingWord(String licensePlate, String[] words) {
		...
		for (String word : words) {
			...
			if (machIt(word, table) && word.length() < min) {
				res = word;
				min = word.length();
			}
		}
	...
	private boolean machIt(String word, int[] table) {
